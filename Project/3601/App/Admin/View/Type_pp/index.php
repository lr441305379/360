...
<table class="table table-hover">
    ...
    <tr>
        <td>{{$v['tpid']}}</td>
        <td>{{$v['tpname']}}</td>
        <td>{{if value="$v['tptype'] eq 0"}}属性{{else}}规格{{endif}}</td>
        <td>{{$v['tpvalue']}}</td>
        <td>
            <a href="{{U('edit',array('tpid'=>$v['tpid']))}}" class="btn btn-sm btn-warning">编辑</a>
            <a href="javascript:if(confirm('确定删除吗?')) location.href='{{U('del',array('tpid'=>$v['tpid']))}}';" class="btn btn-sm btn-danger">删除</a>
        </td>
    ...
    <a  class="btn btn-success"  href="{{U('Type_pp/add',array('shop_type_tid'=>Q('get.tid',0,'intval')))}}">添加属性</a>
