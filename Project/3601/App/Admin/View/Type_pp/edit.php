...
<form action="" method="post">
    ...
    <div class="form-group">
        ...
        <input id="exampleInputEmail1" class="form-control" type="text" placeholder=""  name="tpname" value="{{$data['tpname']}}">
    </div>
    <div id="">
        <label for="">属性类别</label>
        <br />
        <label class="checkbox checkbox-inline" for="">
            <input type="radio" name="tptype" value="0" {{if value="$data['tptype'] eq 0" }} checked {{endif}}>属性
            <input type="radio" name="tptype" value="1" {{if value="$data['tptype'] eq 1" }} checked {{endif}}>规格
        </label>

    </div>
    <div class="form-group">
        <label for="exampleInputEmail1">属性值</label>
        <textarea name="tpvalue" rows="5" cols=""  class="form-control" placeholder="请输入属性值按照逗号分开">{{$data['tpvalue']}}</textarea>
    </div>
    <input type="hidden" name="tpid" value="{{$_GET['tpid']}}">
