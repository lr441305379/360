...
<form action="" method="post" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <img src="<?php echo $oldData['blogo']?>"/>
        <input type="hidden" name="blogo" value="<?php echo $oldData['blogo']?>"/>
    </div>
    <div class="form-group">
        <label for="exampleInputEmail1">品牌排序</label>
        <input id="exampleInputEmail1" class="form-control" type="text" placeholder="" required="" name="bsort" value="<?php echo $oldData['bsort']?>">
    </div>
    <div  class="form-group">
        <label for="exampleInputEmail1">是否热门</label>
        <br />
        <label class="checkbox checkbox-inline" for="">
            <input type="radio" name="bis_hot" value="0" <?php if($oldData['bis_hot']==0){?>
                 checked 
               <?php }?>>不热门
            <input type="radio" name="bis_hot" value="1" <?php if($oldData['bis_hot']==1){?>
                 checked 
               <?php }?>>热门
        </label>
